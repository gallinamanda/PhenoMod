...
for (i in unique(top_fl$species)) {
  ...
  t1<-theme_bw() +
    theme(axis.line = element_line(colour = "black"),
          ...
          axis.title.x = element_text(size=12, margin=margin(5,0,0,0)),
          axis.title.y = element_text(size=12, margin=margin(0,5,0,0)))
  fl_spp_plots[[i]]<-ggplot(spp, aes(year)) +
    labs(x="Year", y = bquote('Observations')) + 
    geom_histogram(binwidth = 1,
                   col="red", 
                   fill="red", 
...
ggsave("fl_year_hist.pdf", marrangeGrob(grobs=fl_spp_plots, nrow=3, ncol=2))


test_data <- data.frame(top_lo[,c(3,2)])
rownames(test_data) <- NULL

transform <- usmap_transform(test_data)
plot_usmap() +
  geom_point(data = transform, aes(x = lon.1, y = lat.1),
             color = "dark green", alpha = 0.25) +
  labs(title = "Leaf-out Observations")
...
for (i in unique(top_lo$species)) {
  ...
  lo_spp_maps[[i]]<-plot_usmap() +
    geom_point(data = transform, aes(x = lon.1, y = lat.1),
               color = "dark green", alpha = 0.25) +
    labs(title = paste(as.character(i)))
}

ggsave("lo_obs_map.pdf", marrangeGrob(grobs=lo_spp_maps, nrow=3, ncol=2))
